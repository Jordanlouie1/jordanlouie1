## About
👋 Hey — I'm Jordan Louie. I'm a Software Engineer at Microchip Technology. I work across embedded and full-stack software with an emphasis on Python and TypeScript, and I enjoy building computer-vision demos, wearable prototypes, and education tools.

- Location: Boston, MA
- Education:
Purdue University — BS Computer Engineering 
University of Colorado - Boulder — MS Electrical and Computer Engineering

## Contact
- LinkedIn: [https://www.linkedin.com/in/jordanlouie1/](https://www.linkedin.com/in/jordanlouie1/)

## Featured Projects
- 🎲 [3DME](https://github.com/Jordanlouie1/3DME) — Make 2D images 3D using Microsoft MoGe and Sam 3D
- 🛠️ [WearableDisplay](https://github.com/Jordanlouie1/WearableDisplay) — Concept for how aomeone would interact with a glasses wearable with an integrated display
- 🧠 [ClassAttentionDetectorv2](https://github.com/Jordanlouie1/ClassAttentionDetectorv2) — Detect who is paying attention by usuing Yolo model to determin who is on their phone
- 🌐 [twinvivo.ai](https://github.com/Jordanlouie1/twinvivo.ai) — Protein Diffusion Models (Boltz2) connected to Phamacokintetic Models (PKsim) to simulate effects of drugs on human body
- 🔍 [Reviu](https://github.com/Jordanlouie1/Reviu) — Annonymous feedback tool

## Tech & Interests
- Languages: Python, C#, C++, TypeScript, JavaScript
- Focus: Computer vision, machine learning, wearable/embedded systems
- Interests: Hackathons, open-source, AR/3D, education technology, hardware prototyping
